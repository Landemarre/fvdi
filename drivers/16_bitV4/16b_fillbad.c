...
static void replace(short *addr, short *addr_fast, int line_add, short *pattern, int x, int y, int w, int h, short foreground, short background)
{
 ...
 /*   if(w>7 && (lpattern[0]==0xFFFFFFFFL)&&(lpattern[1]==0xFFFFFFFFL)&&(lpattern[2]==0xFFFFFFFFL)&&(lpattern[3]==0xFFFFFFFFL)&&(lpattern[4]==0xFFFFFFFFL)&&(lpattern[5]==0xFFFFFFFFL)&&(lpattern[6]==0xFFFFFFFFL)&&(lpattern[7]==0xFFFFFFFFL))
    {
        for(; i < h; i++) {
            for(j = w - 1; j >= 1; j-=2) {
				    		*addr++ = foreground;
