...
static void
blit_or2(PIXEL *src_addr, int src_line_add,
        PIXEL *dst_addr, PIXEL *dst_addr_fast, int dst_line_add,
        int w, int h)
{   /* this version is dependant of 16 bit format !*/
	int i, j;
...
static void
pan_backwards_copyfast(PIXEL *src_addr, int src_line_add,
                   PIXEL *dst_addr, PIXEL *dst_addr_fast, int dst_line_add,
                   int w, int h)
{   /* this version is dependant of 16 bit format !*/
	int i, j;
            ...
            long w, long h, long operation)
{
	...
	if (!to_screen || !dst_addr_fast) {
		...
		if ((src_y == dst_y) && (src_x < dst_x)) {
			...
			switch(operation) {
			case 3:
				if(w>=16) pan_backwards_copyfast(src_addr, src_line_add, dst_addr, 0, dst_line_add, w, h);
				else pan_backwards_copy(src_addr, src_line_add, dst_addr, 0, dst_line_add, w, h);
				break;
			case 7:
        pan_backwards_or(src_addr, src_line_add, dst_addr, 0, dst_line_add, w, h);
			...
			switch(operation) {
			case 3:
				if(w>=16) blit_copyfast(src_addr, src_line_add, dst_addr, 0, dst_line_add, w, h);
				else blit_copy(src_addr, src_line_add, dst_addr, 0, dst_line_add, w, h);
				break;
			case 7:
        if((w%2)==0) blit_or2(src_addr, src_line_add, dst_addr, 0, dst_line_add, w, h);
				else blit_or(src_addr, src_line_add, dst_addr, 0, dst_line_add, w, h);
				break;
			default:
