/*
 * sag_mous.c - Mouse cursor functions
 * This is part of SAGA driver for fVDI
 *
 * Copyright (C) 2017 Vincent Riviere
 * Copyright (C) 2019 Thorsten Otto
 * 32 bits adjustment by Olivier Landemarre
 *
...
static PIXEL backup_data[16*16];
/*
static MFDB mouse_backup_mfdb = {backup_data, 16, 16, 1, 0, PIXEL_SIZE*8, { 0, 0, 0 }};
*/
...
static void hide_mouse(Virtual *vwk)
{
	if (mouse_visible)
	{
		...
		if (hwmouse)
		{
			...
			saga_set_mouse_position(-100, -100);
		} else
		{
			/* Restore the backup */
...
long CDECL c_mouse_draw(Workstation *wk, long x, long y, Mouse *mouse)
{
	...
	if ((long)mouse > 7) /* Set new mouse cursor shape */
	{
		...
		pmouse = mouse;
	/*	if (hwmouse)*/
			saga_set_mouse_sprite(wk, mouse);
	...
	else
	{
		switch ((long)mouse)
		{
		case 0L: /* Move visible */
		case 4L: /* Move visible forced (wk_mouse_forced) */
			show_mouse(vwk, (short)x, (short)y);
			return 1;

		case 1L: /* Move hidden */
		case 5L: /* Move hidden forced (wk_mouse_forced) */
			return 1;
